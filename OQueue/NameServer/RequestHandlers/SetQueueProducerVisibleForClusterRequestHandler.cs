...
{
    public class SetQueueProducerVisibleForClusterRequestHandler : AbstractRequestHandler<SetQueueProducerVisibleForClusterRequest>
    {
        public SetQueueProducerVisibleForClusterRequestHandler(NameServerController nameServerController) : base(nameServerController)
        {
        }

        protected override byte[] Process(IRequestHandlerContext context, SetQueueProducerVisibleForClusterRequest request)
        {
            ...
            requestService.ExecuteActionToAllClusterBrokers(request.ClusterName, remotingClient =>
            {
                ...
                {
                    throw new Exception(string.Format("SetQueueProducerVisible 失败, 失败原因: {0}", Encoding.UTF8.GetString(remotingResponse.ResponseBody)));
                }
            });
            return EmptyBytes;
        }
